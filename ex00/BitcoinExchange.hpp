...
//parseDataBase va lire le fichier data.csv et remplir la map avec les données de la base de données
//parseInput va lire le fichier d'entrée, qui contient les dates et les valeurs pour lesquelles on veut calculer le montant en bitcoins
//parseLine va traiter chaque ligne du fichier d'entrée
//isDateValid va vérifier si une date est valide (format YYYY-MM-DD et valeurs correctes pour l'année, le mois et le jour)
//isValueValid va vérifier si la valeur est valide (positive et pas trop grande)
//getExchangeRate va récupérer le taux de change pour une date donnée, ou le taux de change le plus proche avant cette date si la date n'existe pas dans la base de données
